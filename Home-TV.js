(function () {
    'use strict';

    function startPlugin() {
        var network = new Lampa.Request();

        // 1. Регистрируем компонент
        Lampa.Component.add('hybrid_tv', function (object, exam) {
            var scroll = new Lampa.Scroll({mask:true, over:true});
            var items = [];
            var html = $('<div></div>');

            this.create = function () {
                // Твои источники
                var channels = [
                    { title: 'Первый канал', url: 'https://site-a.com', regex: /(https?:\/\/[^"']+\.m3u8[^"']*)/i },
                    { title: 'ТНТ', url: 'https://site-b.net', regex: /file:"(.*?\.m3u8)"/i }
                ];

                channels.forEach(function (channel) {
                    var card = Lampa.Template.get('button_item', {title: channel.title});
                    card.on('hover:enter', function () {
                        Lampa.Noty.show('Поиск потока...');
                        
                        // Используем прокси, чтобы сайт не блокировал запрос
                        var proxiedUrl = 'http://kulik.uz' + channel.url;

                        network.native(proxiedUrl, function (response) {
                            var match = channel.regex.exec(response);
                            if (match) {
                                var stream = (match[1] ? match[1] : match[0]).replace(/["']/g, '');
                                Lampa.Player.play({ url: stream, title: channel.title });
                            } else { Lampa.Noty.error('Не найдено'); }
                        }, function () { Lampa.Noty.error('Ошибка прокси'); }, false, {dataType: 'text'});
                    });
                    html.append(card);
                    items.push(card);
                });
                scroll.append(html);
            };

            this.render = function () { return scroll.render(); };
            this.active = function () {
                Lampa.Controller.add('content', {
                    toggle: function () { Lampa.Controller.collectionSet(items, html); Lampa.Controller.navigate('content'); },
                    up: function () { Lampa.Controller.toggle('head'); },
                    back: function () { Lampa.Activity.backward(); }
                });
                Lampa.Controller.toggle('content');
            };
        });

        // 2. Метод добавления в меню, который точно сработает
        function addMenu() {
            if ($('.menu__list [data-action="hybrid"]').length) return;

            var item = $('<li class="menu__item selector" data-action="hybrid">' +
                '<div class="menu__ico"><svg viewBox="0 0 24 24" fill="white"><path d="M21 3H3c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h5v2h8v-2h5c1.1 0 1.99-.9 1.99-2L23 5c0-1.1-.9-2-2-2zm0 14H3V5h18v12z"/></svg></div>' +
                '<div class="menu__text">Гибрид ТВ</div>' +
                '</li>');

            item.on('hover:enter', function () {
                Lampa.Activity.push({ title: 'Каналы', component: 'hybrid_tv' });
            });

            // Вставляем сразу после пункта "Сериалы" или просто в начало
            if ($('.menu__list [data-action="tv"]').length) {
                $('.menu__list [data-action="tv"]').after(item);
            } else {
                $('.menu__list').append(item);
            }
        }

        // Повторяем попытки добавить в меню
        for (var i = 1; i <= 5; i++) {
            setTimeout(addMenu, i * 1500);
        }
    }

    if (window.appready) startPlugin();
    else Lampa.Listener.follow('app', function (e) { if (e.type == 'ready') startPlugin(); });
})();
