(function () {
    ...
    $('<style>' +
        ...
        '.home-tv-menu__item { padding: 10px; color: rgba(255,255,255,0.5); font-size: 1.2em; cursor: pointer; }' +
        '.home-tv__list { width: 45%; padding: 0 30px; height: 100%; overflow: hidden; }' +
        ...
        '.htv-page { display: flex; padding: 50px; background: #000; height: 100%; width: 100%; align-items: center; }' +
        ...
        '.htv-page__button { display: inline-block; padding: 20px 60px; background: #f39c12; color: #000; border-radius: 15px; font-weight: bold; font-size: 1.8em; cursor: pointer; text-align: center; }' +
        '.htv-page__button.focus { background: #fff; transform: scale(1.1); }' +
    '</style>').appendTo('body');

    // 2. Компонент СТРАНИЦЫ КАНАЛА (Карточка канала с кнопкой Смотреть)
    Lampa.Component.add('home_tv_view', function (object, exam) {
        var _this = this;
        var html = $('<div class="htv-page">' +
            '<img class="htv-page__logo" src="' + (object.img || 'https://berezka.live') + '">' +
        ...
        this.create = function () {
            ...
            html.find('.htv-page__button').on('hover:enter', function () {
                ...
                // Используем прокси Cub для парсинга ссылки (если нужно)
                network.native('http://cub.watch' + encodeURIComponent(object.url), function (res) {
                    var match = /(https?:\/\/[^"']+\.m3u8[^"']*)/i.exec(res);
                    if (match) {
                        Lampa.Player.play({ url: match[0], title: object.title });
                    } else {
                        // Если m3u8 не найден, пробуем играть прямую ссылку
                        Lampa.Player.play({ url: object.url, title: object.title });
                    }
                }, function () {
                    Lampa.Noty.show('Ошибка сети. Пробуем прямой запуск...');
                    Lampa.Player.play({ url: object.url, title: object.title });
                }, false, { dataType: 'text' });
            });
        };
    ...
    function injectMenu() {
        ...
        if (menu_list.length > 0) {
            ...
            item.on('hover:enter click', function () {
                $('body').removeClass('menu--open'); // Закрываем боковую панель
                Lampa.Activity.push({ 
                    title: 'HOME TV', 
